...
class Solution:
    def findOrder(self, numCourses: int, prerequisites: List[List[int]]) -> List[int]:
        ...
        for course, prereq in prerequisites:
            adj[prereq].append(course)  # prereq -> courses that need it
        
        visited = set()
        path = set()      # Current DFS path
        order = []       # Store topological order
        ...
        def dfs(course):
            ...
            # Remove from path and mark as fully explored
            path.remove(course)
            visited.add(course)
            order.append(course)
            return True
        ...
        if len(order) < numCourses:
            remaining = set(range(numCourses)) - visited
